...
def main(root):
    ...
    # Save times and states
    np.save(os.path.join(data_dir, ground_truth_filename), {"times": times, "states": states})
...
def simulate():
    ...
    ground_truth = np.load(os.path.join(data_dir, ground_truth_filename), allow_pickle=True).item()
...
def plotTrajectory():
    ...
    ax.set_title("Missile Trajectory in ECF Coordinates")
    ax.legend()
